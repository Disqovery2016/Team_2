...
function curstud($sid)
 {
   try
    ...
    $curcnt = $conn->prepare("SELECT COUNT(*) FROM schools JOIN students ON schools.sid = students.schl_id JOIN rfid ON students.stid = rfid.imei AND schools.sid = :schoolid");
 ...
 function curtech($sid)
 {
   try
    ...
    require 'connect.php';
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $curcnt = $conn->prepare("SELECT COUNT(*) FROM schools JOIN workers ON schools.sid = workers.wkschl_id JOIN rfid ON workers.wkid = rfid.imei AND schools.sid = :schoolid");
    $curcnt->bindParam(':schoolid', $sid, PDO::PARAM_STR);
    $curcnt->execute();
    $curcntres=$curcnt->fetchColumn();
    }
